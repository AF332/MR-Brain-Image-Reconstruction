...
def conv_block(inputs=None, n_filters=90, batch_norm=False, dropout_prob=0.4):
    ...
    convolutional_2 = SeparableConv2D(n_filters, 2, padding='same', kernel_initializer='HeNormal')(convolutional_1)
    if batch_norm:
...
def encoder_block(inputs=None, n_filters=90, batch_norm=True, dropout_prob=0.4): # added a higher dropout rate
...
def Unet(input_size=(256, 256, 1), n_filters=90, n_classes=2, batch_norm=False, dropouts= [0.4]*13):
...
# compile model
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.00005),  # used optimizer with chosen learning rate
...
history = model.fit(x = train_input, y = train_target, batch_size = 32, epochs = 15      , verbose = 1, validation_split = 0.2)
...
model.save('c://Users//2418015//Downloads//Models//V1') # Change the path to specify a place to save the trained model
