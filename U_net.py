...
def undersample(kspace, undersamplefactor):
    # Get the shape of the k-space
    height, width = kspace.shape
    # Calculate the number of pixels to keep
    num_pixels_undersampled = int(height * width * (1 - undersamplefactor))
    # Generate random pixel indices to keep
    random_pixels = np.random.choice(height * width, num_pixels_undersampled, replace=False)
    # Convert the 1D indices to 2D indices
    random_rows = random_pixels // width
    random_cols = random_pixels % width
    # Create an array of zeros
    undersample_kspace = np.zeros_like(kspace)
    # Place the undersampled pixels in the array
    undersample_kspace[random_rows, random_cols] = kspace[random_rows, random_cols]
    return undersample_kspace
#This is makes random images each time, not machine deoendent each time you run it will be different.
...
def read_data(path, keyname):
  ...
  for i in range(int(len(h5name))): # Repeat for every file in folder from path
...
def read_source_images_noise(path, mean, standard_deviation):
  ...
  for i in range(data_length): # Repeat for the number of files in folder
    ...
    for j in range(int(num_acq/2)):  # Repeat for number of acquisitions in each file/2 (cutting out non brain structures in the images)
      ...
      un_im_noise = undersample(im_noise, 0.5)
...
def encoder_block(inputs=None, n_filters=64, batch_norm=True, dropout_prob=0.4): # added a higher dropout rate
...
def decoder_block(expansive_input, skip_con, n_filters, batch_norm=True, dropout_prob=0.4):
    up_samp = Conv2DTranspose(n_filters, 4, strides=2, padding='same', kernel_initializer='HeNormal')(expansive_input)
...
def Unet(input_size=(256, 256, 1), n_filters=64, n_classes=2, batch_norm=True, dropouts= [0.4]*12):
    ...
    encoder_block_6 = encoder_block(encoder_block_5[0], n_filters * 32, batch_norm, dropout_prob=dropouts[5])

    bridge = conv_block(encoder_block_6[0], n_filters * 64, batch_norm, dropout_prob=dropouts[6])

    decoder_block_6 = decoder_block(bridge, encoder_block_6[1], n_filters * 32, batch_norm, dropout_prob=dropouts[7])
    decoder_block_5 = decoder_block(decoder_block_6, encoder_block_5[1], n_filters * 16, batch_norm, dropout_prob=dropouts[8])
    decoder_block_4 = decoder_block(decoder_block_5, encoder_block_4[1], n_filters * 8, batch_norm, dropout_prob=dropouts[9])
    decoder_block_3 = decoder_block(decoder_block_4, encoder_block_3[1], n_filters * 4, batch_norm, dropout_prob=dropouts[10])
    decoder_block_2 = decoder_block(decoder_block_3, encoder_block_2[1], n_filters * 2, batch_norm, dropout_prob=dropouts[11])
    decoder_block_1 = decoder_block(decoder_block_2, encoder_block_1[1], n_filters, batch_norm, dropout_prob=dropouts[12])
...
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001),  # used optimizer with chosen learning rate
              loss='mse',  # loss function (what if we used a different loss function?
              metrics=['mse', 'mae'])

"""The ephocs refer to the iterations. During each of this, the model processes the entire training dataset. """ # increase the epochs might help it
history = model.fit(x = train_input, y = train_target, batch_size = 32, epochs = 7      , verbose = 1, validation_split = 0.2)
...
model.save(r'E:\Data\nipple') # Change the path to specify a place to save the trained model
